...
namespace DatabaseSyncService.Workers
{
    public class Worker7 : BackgroundService
    {
        ...
        {
            if (string.IsNullOrEmpty(_workerModel.Worker7))
                return;
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    await DataSyncHelper.Sync(_workerModel.MasterDB, _workerModel.Worker7);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex.Message);
                }
                _logger.LogInformation($"{typeof(Worker7)} running at: {DateTime.Now}");
