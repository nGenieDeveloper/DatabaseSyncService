...
namespace DatabaseSyncService.Workers
{
    public class Worker8 : BackgroundService
    {
        ...
        {
            if (string.IsNullOrEmpty(_workerModel.Worker8))
                return;
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    await DataSyncHelper.Sync(_workerModel.MasterDB, _workerModel.Worker8);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex.Message);
                }
                _logger.LogInformation($"{typeof(Worker8)} running at: {DateTime.Now}");
