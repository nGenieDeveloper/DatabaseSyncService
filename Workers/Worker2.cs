...
namespace DatabaseSyncService.Workers
{
    public class Worker2 : BackgroundService
    {
        ...
        {
            if (string.IsNullOrEmpty(_workerModel.Worker2))
                return;
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    await DataSyncHelper.Sync(_workerModel.MasterDB, _workerModel.Worker2);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex.Message);
                }
                _logger.LogInformation($"{typeof(Worker2)} running at: {DateTime.Now}");
